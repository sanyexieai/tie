...
const sourceForPage = computed(() => new Map((store.workspace?.sources ?? []).map((source) => [source.id, source])))
...
<template>
  <main class="tag-view">
    ...
    <section class="tag-content">
      ...
      <select v-model="store.tagStorageSourceId" class="tag-source-filter" aria-label="筛选存储源"><option :value="null">全部存储源</option><option v-for="source in store.workspace?.sources" :key="source.id" :value="source.id">{{ source.kind === 'smb' ? 'SMB · ' : '本地 · ' }}{{ source.name }}</option></select>
      <div class="tag-index">
      ...
      <div v-if="store.selectedTag" class="tagged-pages">
        <button v-for="page in store.taggedPages" :key="page.id" @click="store.openPage(page.id)">
          <strong>{{ page.title }}</strong><span>{{ sourceForPage.get(page.storageSourceId)?.kind === 'smb' ? 'SMB · ' : '本地 · ' }}{{ sourceForPage.get(page.storageSourceId)?.name ?? '未知来源' }} · {{ page.updatedAt.slice(0, 10) }}</span>
        </button>
