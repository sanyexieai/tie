...
interface GraphNode { id: string; label: string; kind: NodeKind; x: number; y: number; pageId?: string; tag?: string }
interface GraphEdge { from: string; to: string; kind: 'link' | 'tag' | 'content' }

const store = useWorkspaceStore()
const filter = ref('')
const graph = computed(() => {
  const query = filter.value.trim().toLocaleLowerCase()
  const pages = store.pages.filter((page) => !page.deletedAt && (!query || `${page.title} ${page.tags.join(' ')}`.toLocaleLowerCase().includes(query))).slice(0, 60)
  ...
  const titleNodes: GraphNode[] = placeRing(pages.map((page) => ({ id: page.id, label: page.title, kind: 'title' as const, pageId: page.id })), 360, 250)
  ...
  const edges: GraphEdge[] = [
    ...store.links.filter((link) => pageIds.has(link.fromPageId) && pageIds.has(link.toPageId)).map((link) => ({ from: link.fromPageId, to: link.toPageId, kind: 'link' as const })),
...
<template>
  <main class="global-graph-view">
    ...
    <section class="global-graph-content">
      <div class="graph-view-heading"><div><p class="eyebrow">知识图谱</p><h1>关联网络</h1></div><input v-model="filter" placeholder="筛选页面或标签…" /></div>
      <div class="global-graph-legend"><span class="legend-title">● 标题页</span><span class="legend-tag">■ 标签</span><span class="legend-content">◆ 内容抽象</span><small>{{ graph.nodes.length }} 个节点 · {{ graph.edges.length }} 条关系</small></div>
      <div class="global-graph-canvas">
        <svg v-if="graph.nodes.length" viewBox="0 0 1000 680" aria-label="全局知识图谱">
          <line v-for="edge in graph.edges" :key="`${edge.kind}-${edge.from}-${edge.to}`" :class="edge.kind" :x1="graph.nodes.find((node) => node.id === edge.from)?.x" :y1="graph.nodes.find((node) => node.id === edge.from)?.y" :x2="graph.nodes.find((node) => node.id === edge.to)?.x" :y2="graph.nodes.find((node) => node.id === edge.to)?.y" />
          <g v-for="node in graph.nodes" :key="node.id" class="global-graph-node" :class="node.kind" @click="selectNode(node)">
