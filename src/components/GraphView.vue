...
const graph = computed(() => {
  ...
  const titleNodes: GraphNode[] = placeRing(pages.map((page) => ({ id: page.id, label: page.title, kind: 'title' as const, pageId: page.id, sourceKind: store.allSources.find((source) => source.id === page.storageSourceId)?.kind ?? 'local' })), 360, 250)
...
<template>
  <main class="global-graph-view">
    <header class="editor-header"><div class="breadcrumbs"><span>{{ store.workspace?.name ?? '我的知识库' }}</span><span>›</span><span>知识图谱</span></div></header>
    <section class="global-graph-content">
      <div class="graph-view-heading"><div><p class="eyebrow">知识图谱</p><h1>关联网络</h1></div><div class="graph-filters"><input v-model="filter" placeholder="筛选页面或标签…" /><select v-model="sourceFilter" aria-label="筛选图谱存储源"><option :value="null">全部存储源</option><option v-for="source in store.allSources" :key="source.id" :value="source.id">{{ source.kind === 'backend' ? '后台 · ' : source.kind === 's3' ? 'S3 · ' : source.kind === 'smb' ? 'SMB · ' : '本地 · ' }}{{ source.name }}</option></select></div></div>
      <div class="global-graph-legend"><span class="legend-title">● 本地标题页</span><span class="legend-smb">● SMB 标题页</span><span class="legend-backend">● 后台标题页</span><span class="legend-tag">■ 标签</span><span class="legend-content">◆ 内容抽象</span><span class="legend-cross-source">— 跨源链接</span><small>{{ graph.nodes.length }} 个节点 · {{ graph.edges.length }} 条关系</small></div>
