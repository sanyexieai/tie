...
- 桌面端可注册多个存储源：任意本地目录或操作系统已挂载的 SMB 共享目录，以及 S3/MinIO、自定义后台。每个页面有一个**协作主源**（`storage_source_id`），并可额外绑定**备份镜像**（`storage_source_ids`）。**协作主源只能是云端**（S3/后台）；本机目录仅可作备份。**跨端同步只认云端源**：同一 MinIO 用 endpoint+bucket 指纹生成稳定源 id；写入云端时会剥离本机目录 id，避免异机幽灵绑定。日常保存与附件只写入协作主源；多客户端应共用同一云端主源。本机目录作为备份镜像时需手动「同步到备份」。不同源的页面仍可在同一页面树、链接和图谱中关联。旧版单目录配置会自动迁移为一个存储源。
- 编辑器顶栏可勾选备份镜像、指定协作主源，或一键「同步到备份」；取消绑定会删除该源上的副本，不影响其他源。将页面「迁移」到单一目标源时仍会移除其他绑定。
...
- 桌面端支持「从文件打开」：选择本地 `.md` / `.markdown` 后直接打开为页面。若文件已在某存储源目录下则复用该源；否则会以文件所在目录（或向上找到的含 `pages/` 的目录）自动注册本地工作区，再打开或导入页面。
...
- **跨源迁移**：可在编辑器中切换/迁移协作主源；file ↔ S3 迁移时保留历史版本与 `tie://asset/` 图片附件。
- **备份镜像**：绑定额外源作灾备；不参与每次保存扇出，用「同步到备份」推送正文与附件。
